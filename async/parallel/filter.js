...
module.exports = (collection, iteratee, output, ctx, taskLimit) => {
  return collection ? (
    iteratee = iterateeNormalize(iteratee),
    isArray(output) || isArray(collection)
      ? filterEach(collection, iteratee, output || [], ctx, taskLimit)
      : filterEach(entries(collection), (line) => {
        return iteratee.call(ctx, line[1], line[0], collection);
      }, [], ctx, taskLimit).then((_entries) => fromPairs(_entries, output))
